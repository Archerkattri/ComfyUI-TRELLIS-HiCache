    ...
    class Pipe:
        def __init__(self):
            self.models = {"sparse_structure_flow_model": DummyDiT(),
                           "slat_flow_model": DummyDiT()}
    p = Pipe()
    patched = apply_hicache(p, method="hermite", interval=2, stages="both")
    # original untouched
    assert not getattr(p.models["slat_flow_model"], "_hicache_is_patch", False)
    assert getattr(patched.models["slat_flow_model"], "_hicache_is_patch", False)
    assert patched.models is not p.models
    # remove restores
    clean = remove_hicache(patched)
    assert not getattr(clean.models["slat_flow_model"], "_hicache_is_patch", False)
...
def test_apply_never_nests():
    ...
    inner = b.models["slat_flow_model"].inner
