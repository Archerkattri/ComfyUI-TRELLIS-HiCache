...
STAGES = ("both", "sparse_structure", "slat")

# TRELLIS' two flow DiTs, keyed in pipeline.models.
_STAGE_KEYS = {
    "sparse_structure": ["sparse_structure_flow_model"],
    "slat": ["slat_flow_model"],
    "both": ["sparse_structure_flow_model", "slat_flow_model"],
}
...
class HiCacheModelPatch(torch.nn.Module):
    ...
    def __init__(self, model: Optional[torch.nn.Module], *, method: str = "hermite",
        ...
        super().__init__()
        # ``model`` may be None here: GGUF / lazy pipelines leave
        # pipeline.models[key] empty at patch time and materialize the flow DiT
        # later, inside the sampler. A real nn.Module is registered as a submodule
        # (so .to()/.state_dict()/.parameters() recurse); a None/lazy deferred module is
        # kept as a plain attribute so __getattr__ never dead-ends. See _set_inner.
    ...
    def cfg_mode(self) -> str:
        """TRELLIS uses split conditional/unconditional model forwards."""
    ...
    def telemetry(self) -> Dict[str, Any]:
        """Detached actual-method/fallback counters with per-branch detail."""
    ...
    def budget_manifest(self) -> Dict[str, Any]:
        ...
        if self._budget_runtimes:
            ...
            return {
                "schema": "hicache-pp.trellis-budget-manifest.v1",
                "counts": counts,
                "branches": branches,
            }
    ...
    def __getattr__(self, name: str):
        ...
        if inner is None:
            raise AttributeError(
                ...
                f"flow model is not loaded yet (inner is None). Lazy / GGUF pipelines "
                f"materialize the DiT inside the sampler; the patch binds it "
                f"automatically when it is assigned into pipeline.models, or call "
    ...
    def _ensure_budget_runtime(
        ...
        if runtime is None or runtime.identity.fingerprint != identity.fingerprint:
            runtime = CacheBudgetRuntime(
                self.budget,
                identity,
                source_digest=stable_digest({"adapter": "ComfyUI-TRELLIS-HiCache", "contract": "budget-v1"}),
    ...
    def forward(self, latent_model_input: Any, timestep: Any,
        ...
        # Stock TRELLIS does not pass these markers, so the timestep heuristic
        ...
        if inner is None:
            raise RuntimeError(
                ...
                "model is not loaded (inner is None). For lazy / GGUF pipelines, apply "
...
class _LazyPatchDict(dict):
    ...
    GGUF / lazy pipelines leave a flow-model slot empty (``None``) at patch time
    and assign the real DiT later, from inside the sampler, via
    ``pipeline.models[key] = model``. With a plain dict that would overwrite the
    :class:`HiCacheModelPatch` wrapper and silently disable the cache. This
    subclass instead *binds* the freshly loaded model into the existing patch, so
    acceleration survives the lazy load with no user action. It is a ``dict``
    subclass, so ``isinstance(models, dict)`` checks still pass.
    """
...
def apply_hicache(pipeline: Any, *, method: str = "hermite", interval: int = 3,
    ...
    for key in keys:
        ...
        dict.__setitem__(patched.models, key, HiCacheModelPatch(
            ...
            stage_id=key, budget=budget, max_horizon=max_horizon,
